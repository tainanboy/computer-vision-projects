...
def build_dc_classifier():
    ...
    return nn.Sequential(
        ...
        ######### TO DO ###########
        ###########################
        Unflatten(batch_size, 1, 28, 28),    # (N, 1, 28, 28)
        nn.Conv2d(1, 32, kernel_size=5, stride=1, padding=0, bias=True),    # (N, 32, 24, 24)
        nn.LeakyReLU(0.01),
        nn.MaxPool2d(2, stride=2),    # (N, 32, 12, 12)
        nn.Conv2d(32, 64, kernel_size=5, stride=1, padding=0, bias=True),    # (N, 64, 8, 8)
        nn.LeakyReLU(0.01),
        nn.MaxPool2d(2, stride=2),    # (N, 64, 4, 4)
        Flatten(),    # (N, 1024)
        nn.Linear(1024, 1024),  # bias=True is default
        nn.LeakyReLU(0.01),
        nn.Linear(1024, 1)  # bias=True is default
    )
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    return nn.Sequential(
        ...
        ######### TO DO ###########
        ###########################
        nn.Linear(noise_dim, 1024),  # bias=True is default    # (N, 1024)
        nn.ReLU(),
        nn.BatchNorm1d(1024),
        nn.Linear(1024, 6272),  # bias=True is default    # (N, 6272)
        nn.BatchNorm1d(6272),
        Unflatten(batch_size, 128, 7, 7),    # (N, 128, 7, 7)
        nn.ConvTranspose2d(128, 64, kernel_size=4, padding=1, stride=2),    # (N, 64, 14, 14)
        nn.ReLU(),
        nn.BatchNorm2d(64),
        nn.ConvTranspose2d(64, 1, kernel_size=4, padding=1, stride=2),    # (N, 1, 28, 28)
        nn.Tanh(),
        Flatten()    # (N, 784)
    )
